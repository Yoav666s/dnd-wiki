---
title: "Way of the Sands"
index:
  - way-of-the-sands
permalink: /classes/monk/way-of-the-sands/
excerpt: "Monks of this monastic tradition are shaped by exposure to unforgiving landscapes where heat, wind, and scarcity erode weakness and reward precision."
subclass: monk
alt_version: warrior-of-the-sands
source: "Nomad's Guide to Cortenmaas"
header:
  overlay_image: /assets/images/classes/monk/sands/header.png
  teaser: /assets/images/classes/monk/sands/header.jpg
---
Monks of this monastic tradition are shaped by exposure to unforgiving landscapes where heat, wind, and scarcity erode weakness and reward precision. Their discipline reflects the shifting nature of the desert itself--fluid in motion, sudden in violence, and relentless in endurance. These monks learn to read the terrain as an extension of their own bodies, gliding across loose ground without hesitation and turning every grain beneath their feet into an advantage.

# One with the Wastes
Starting at 3rd level, your martial arts techniques adapt to the harsh reality of the desert. You gain the following abilities.

- You ignore difficult terrain composed of sand or loose earth.
- Whenever you deal slashing damage to a creature, you can roll the damage die twice and use either roll against the target.
- Whenever you make an attack granted by your Flurry of Blows, you can attack with a finesse weapon that deals slashing damage in place of an unarmed strike.

# Dirty Fighting
Starting at 6th level, once per turn and immediately before you take the Attack action while standing on ground composed of sand or loose earth, you can expend 1 Ki point to force a creature you can see within 5 feet of you to make a Dexterity saving throw. On failure, the creature has the Blinded condition until the end of its next turn. You can also use this feature anywhere by expending a handful of sand from a pouch you carry, but you must have a free hand to do so.

# Crescent Riposte
Starting at 11th level, when a creature misses you with a melee attack, you can take your reaction to make an Unarmed Strike or melee weapon attack with a finesse weapon that deals slashing damage against the attacker.

# Deep Cuts
Starting at 17th level, whenever you deal slashing damage to a creature, you deal additional damage equal to your Wisdom modifier (minimum of 1).

[:classes/monk/sands:]
